...
public partial class CMSModules_MediaLibrary_Controls_MediaLibrary_MediaFileMultipleImport : CMSAdminControl
{
    ...
    public string SiteName
    {
        get
        {
            ...
            {
                return CMSContext.CurrentSiteName;
            }
        }
    }
    ...
    private void UpdateImportIndex()
    {
        ...
        ltlImportFilesNo.Text = String.Format("<div style=\"white-space:nowrap;\">{0}</div>", string.Format(FILES_NUMBERS_TEXT, ImportCurrFileIndex, ImportFilesNumber));
    }
    ...
    private void UpdateImportForm()
    {
        ...
        if (!String.IsNullOrEmpty(nextFileName))
        {
            ...
            if (!MediaLibraryHelper.IsExtensionAllowed(ext.TrimStart('.')))
            {
                lblError.Text = string.Format(GetString("attach.notallowedextension"), ext, MediaLibraryHelper.GetAllowedExtensions(SiteName).TrimEnd(';').Replace(";", ", "));
                lblError.Visible = true;
            ...
            InitializeImportBreadcrumbs(URLHelper.GetSafeFileName(nextFileName, SiteName));
        }
    }
